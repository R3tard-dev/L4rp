...
public class ESP extends Module implements ModuleSettingsRenderer {
    ...
    private void render(DrawContext context, float tickDelta) {
        ...
        for (PlayerEntity target : players) {
            ...
            int[] screen = projectBox(target, tickDelta, screenW, screenH);
    ...
    private void drawSkeleton(DrawContext ctx, PlayerEntity target, float tickDelta,
        ...
        int col = boxColour;
        drawBone(ctx, head,  neck,  screenW, screenH, col);
        drawBone(ctx, neck,  chest, screenW, screenH, col);
        drawBone(ctx, chest, waist, screenW, screenH, col);
        drawBone(ctx, chest, lSh,   screenW, screenH, col);
        drawBone(ctx, chest, rSh,   screenW, screenH, col);
        drawBone(ctx, lSh,   lEl,   screenW, screenH, col);
        drawBone(ctx, rSh,   rEl,   screenW, screenH, col);
        drawBone(ctx, waist, lHp,   screenW, screenH, col);
        drawBone(ctx, waist, rHp,   screenW, screenH, col);
        drawBone(ctx, lHp,   lKn,   screenW, screenH, col);
        drawBone(ctx, rHp,   rKn,   screenW, screenH, col);
        drawBone(ctx, lKn,   lFt,   screenW, screenH, col);
        drawBone(ctx, rKn,   rFt,   screenW, screenH, col);
    }

    private void drawBone(DrawContext ctx, Vec3d a, Vec3d b,
    ...
    private int[] projectBox(PlayerEntity target, float tickDelta, int screenW, int screenH) {
        Vec3d pos = target.getLerpedPos(tickDelta);
        double hw = target.getWidth() / 2.0;
        double h  = target.getHeight();

        Vec3d[] corners = {
            new Vec3d(pos.x - hw, pos.y,     pos.z - hw),
            new Vec3d(pos.x + hw, pos.y,     pos.z - hw),
            new Vec3d(pos.x - hw, pos.y + h, pos.z - hw),
            new Vec3d(pos.x + hw, pos.y + h, pos.z - hw),
            new Vec3d(pos.x - hw, pos.y,     pos.z + hw),
            new Vec3d(pos.x + hw, pos.y,     pos.z + hw),
            new Vec3d(pos.x - hw, pos.y + h, pos.z + hw),
